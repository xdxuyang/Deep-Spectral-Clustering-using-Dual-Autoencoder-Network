...
from core.vaeConv2d import encoder,vae,classfier
...
def get_data(params, data=None):
    ...
    vae.load_weights('/home/yangxu/workspce/infomax-master/vae_mnist2.h5')
    if params.get('use_code_space'):

        # for i, d in enumerate(all_data):
        #     all_data[i] = embed_data(d, dset=params['dset'])
        all_data[0] = encoder.predict(all_data[1])
        all_data[0] = classfier.predict(all_data[0])
        all_data[1] = encoder.predict(all_data[1])
        # all_data[1] = classfier.predict(all_data[1])
        all_data[3] = encoder.predict(all_data[3])
        # all_data[3] = classfier.predict(all_data[3])
        all_data[5] = encoder.predict(all_data[5])
        # all_data[5] = classfier.predict(all_data[5])
        all_data[2] = embed_data(all_data[2], dset=params['dset'])
        all_data[4] = embed_data(all_data[4], dset=params['dset'])
        all_data[6] = embed_data(all_data[6], dset=params['dset'])
    else:
    ...
    if 'siamese' in params['affinity']:
        ...
        pairs_train = pairs_train_unlabeled
        dist_train = dist_train_unlabeled
        pairs_val = pairs_val_unlabeled
        dist_val = dist_val_unlabeled
